...
#
# Newest version of the perl script can be found at http://www.ekstroem.com/nwn/nwn.pl
################
...
my @DAMAGETYPES = ("Physical", "Acid", "Electrical", "Fire", "Cold", "Sonic", "Magical", "Divine", "Positive", "Negative");
my @DAMAGETYPESIMM = ("Bludgeoning", "Piercing", "Slashing", "Acid", "Electrical", "Fire", "Cold", "Sonic", "Magical", "Divine", "Positive", "Negative");
my @COLS = ("orange", "blue", "yellow", "white", "darkgray", "pink", "green", "red", "lightblue", "purple");
...
$mw ->title("NWN logger v" .$version . ". --- by Claus Ekstrom 2008.");
...
$frm_othertimers -> Label(-textvariable => \$othertimers, -foreground=>$col_cold, -background=>"black") ->pack();
...
#	   $hitpercentage{$attacker} = 1 / $swings{attacker};
	...
	#
	   ...
	   # Start death timer if it was the toon that died
	   if ($2 eq $toon) {
	       $deaths++;
	       $lastkiller = $1;
	   } 
	...
	if (/Your Eternal Return spell fires, preventing the life from leaving your body!/) {
       ...
       if (/^    (Bludgeoning|Piercing|Slashing|Magical|Acid|Cold|Divine|Electrical|Fire|Negative|Positive|Sonic): (-?\d+)%$/) {
	   $immunities{$1} = $2;
	   print_immunities();
	   next;
       }
...
#	    print "Found $toonname in this line:>>>$_<<<\n";
	...
	# Effects 
        #
	# This one registers who the effects concern
...
sub html_summary {
	...
	if ($shamelessadvertising ==1) {
	    print SAVEFILE "Yet another successful Nessus run kept under control by <a href=\"http://highergroundpoa.proboards3.com/index.cgi?board=software&action=display&thread=10946\">NWN-logger</a> v" . $version . " by Claus Ekstroem.</p>\n";
	}
	else {
